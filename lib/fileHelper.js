...
function FileHelper() {
    ...
    return {
        ...
        createMakeDownFile: function _createMakeDownFile(folderPath, parentNode, imageConfigs, spaceKey) {
            var self = this;
            var fileName = parentNode.title + ".md";
            var filePath = util.format("%s/%s", folderPath, fileName);
            var CONFLUENCE_URL_PATH = '/display';
            if (spaceKey) {
                CONFLUENCE_URL_PATH += '/' + spaceKey + '/';
            }
            ...
            var converters = [{
                ...
                replacement: function(content, node) {
                    ...
                    //case 1: if url contains /display/{spaceKey}/
                    const Confluence = require('./Confluence');
                    var config = self.loadConfigration();
                    var confluence = Confluence(config);
                    var parentPathPieces = folderPath.replace(config.dataPath).split('/');
                    var parentPathWithoutSpaceFolder = parentPathPieces.slice(2, parentPathPieces.length).join('/');
                    if(href.indexOf(CONFLUENCE_URL_PATH) > -1) {
                        ...
                        //     }
                        // }
                        var titleParent = parentPathWithoutSpaceFolder;
                        href = href
